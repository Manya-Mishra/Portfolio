# 🚀 Digital Portfolio

## 📌 Overview

A modern and responsive **Digital Portfolio Website** built to showcase my skills, projects, education, certifications, and achievements as an aspiring **Data Analyst and AI & ML professional**. <br>The portfolio serves as a central hub for recruiters and hiring managers to explore my technical expertise, project work, and professional journey.

## 🌟 Features

* Responsive and mobile-friendly design
* Smooth scrolling single-page layout
* Professional Hero Section with introduction
* About Me section
* Technical Skills showcase
* Featured Projects with descriptions and GitHub links
* Education and Certifications section
* Contact Form for easy communication
* Clean and modern UI/UX

## 🛠️ Technologies Used

* HTML5
* CSS3
* VS Code
* Git & GitHub
* GitHub Pages (Deployment)

## 📂 Sections Included

### Home

Professional introduction and career objective.

### About

Overview of my academic background, interests, and career goals.

### Skills

* Python
* SQL
* Power BI
* Tableau
* Excel
* Machine Learning
* Data Analysis
* Git & GitHub

### Projects

#### 📊 Pizza Sales Analysis

Analyzed sales trends and business KPIs using SQL and Power BI dashboards.

#### 📈 Customer Churn Analysis

Performed exploratory data analysis and predictive modeling on telecom customer data.

#### 🌍 AQI Prediction System

Developed a machine learning model to predict Air Quality Index using data from 26 Indian cities and 29,531 daily records.

### Experience

* Deloitte Australia Data Analytics Virtual Experience – Forage
* TATA Data Analytics Virtual Experience – Forage

### Education

🎓  Master of Computer Applications
    (AI & ML)
    Galgotias University, Greater Noida
    CGPA 9.65 • Expected 2027

🎓  Bachelor of Computer Applications
    Shri Ramswaroop Memorial University, Barabanki
    CGPA 8.01 • 2025

### Certifications

* Python (Basic) – HackerRank
* SQL (Intermediate) – HackerRank
* Generative AI Foundations — AWS Academy Graduate
* Machine Learning Foundations — AWS Academy Graduate
* DBMS – Great Learning

### Contact

Visitors can connect with me through email, LinkedIn, and GitHub.

## 🎯 Purpose

This portfolio was created to:

* Showcase my technical and analytical skills
* Highlight real-world data analytics projects
* Demonstrate web development capabilities
* Build a professional online presence

## 🔗 Live Demo

[Add your deployed GitHub Pages link here.](https://manya-mishra.github.io/Portfolio/)

## 📬 Connect With Me

* LinkedIn: [Manya-Mishra-linkedin](https://www.linkedin.com/in/manya-mishra-2986a324b/)
* GitHub: [Manya-Mishra-github](https://github.com/Manya-Mishra)
* Email: manyamishra82@gmail.com

---

⭐ If you found this project useful, feel free to star the repository.
